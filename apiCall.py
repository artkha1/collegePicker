...
def fields_for_year(year):
    if year == LATEST_YEAR:
        return fields
    return fields.replace("latest.", f"{year}.")
...
def get_col_data(page, year):
    p = {
        "school.operating": "1",
        "fields": fields_for_year(year),
        "page": page,
        "api_key": key
    }
...
def callAPI(year):
    first_response = get_col_data(0, year)
...
if __name__ == '__main__':
    ...
    for year in YEARS:
        print(f"Fetching data from College Scorecard API for {year}...")
        df = callAPI(year)
        print(f"Fetched {len(df)} colleges for {year}.")

        print(f"Building dynamic table for {year}...")
    ...
    print("Done inserting historical dynamic data.")